...
int bq2589x_set_otg_volt(struct bq2589x *bq, int volt)
{
	...
	if (bq->part_no == SC89890H) {
		...
		if (volt > SC89890H_BOOSTV_BASE + (BQ2589X_BOOSTV_MASK >>
		...
		if (volt > BQ2589X_BOOSTV_BASE + (BQ2589X_BOOSTV_MASK >>
						  BQ2589X_BOOSTV_SHIFT) *
							 BQ2589X_BOOSTV_LSB)
			volt = BQ2589X_BOOSTV_BASE +
			       (BQ2589X_BOOSTV_MASK >> BQ2589X_BOOSTV_SHIFT) *
...
int bq2589x_reserved_disable(struct bq2589x *bq)
{
	...
	return bq2589x_update_bits(bq, BQ2589X_REG_03, BQ2589X_RESERVED_MASK,
				   val);
}
...
static int bq2589x_bat_compensation(struct bq2589x *bq, int bat_comp)
{
	...
	return bq2589x_update_bits(bq, BQ2589X_REG_08, BQ2589X_BAT_COMP_MASK,
...
static int bq2589x_voltage_clamp(struct bq2589x *bq, int vclamp)
{
	...
	return bq2589x_update_bits(bq, BQ2589X_REG_08, BQ2589X_VCLAMP_MASK,
...
int bq2589x_force_dpdm(struct bq2589x *bq)
{
	...
	if (bq->part_no == SC89890H) {
		ret = bq2589x_read_byte(bq, &data, BQ2589X_REG_0B);
		if ((data & BQ2589X_VBUS_STAT_MASK) >>
			    BQ2589X_VBUS_STAT_SHIFT ==
		    BQ2589X_VBUS_MAXC) {
			bq2589x_write_byte(bq, BQ2589X_REG_01,
					   SC89890H_FORCE_DPDM1);
			msleep(30);
			bq2589x_write_byte(bq, BQ2589X_REG_01,
					   SC89890H_FORCE_DPDM2);
			msleep(30);
		}
	}
	dev_err(bq->dev, "%s \n", __func__);
	return bq2589x_update_bits(bq, BQ2589X_REG_02, BQ2589X_FORCE_DPDM_MASK,
				   val);
}
...
int bq2589x_force_ico(struct bq2589x *bq)
{
	...
	ret = bq2589x_update_bits(bq, BQ2589X_REG_09, BQ2589X_FORCE_ICO_MASK,
				  val);
...
int bq2589x_use_absolute_vindpm(struct bq2589x *bq, bool enable)
{
	...
	if (enable)
		...
		val = BQ2589X_FORCE_VINDPM_DISABLE
		      << BQ2589X_FORCE_VINDPM_SHIFT;

	ret = bq2589x_update_bits(bq, BQ2589X_REG_0D, BQ2589X_FORCE_VINDPM_MASK,
				  val);
...
void bq2589x_dump_regs(struct bq2589x *bq)
{
	...
	for (addr = 0x0; addr <= 0x14; addr++) {
		...
		sprintf(temp_buf + strlen(temp_buf), "0x%02X[0x%02X],", addr,
			regs[addr]);
...
int charger_request_dpdm(struct bq2589x *charger,
			 int enable) //enable == false, enable Hiz
{
	int rc = 0;
	...
	if (!charger->dpdm_reg &&
	    of_get_property(charger->dev->of_node, "dpdm-supply", NULL)) {
...
static int bq2589x_parse_dt(struct device *dev, struct bq2589x *bq)
{
	...
	bq->cfg.enable_auto_dpdm =
		of_property_read_bool(np, "ti,bq2589x,enable-auto-dpdm");
	bq->cfg.enable_term =
		of_property_read_bool(np, "ti,bq2589x,enable-termination");
	bq->cfg.enable_ico = of_property_read_bool(np, "ti,bq2589x,enable-ico");
	bq->cfg.use_absolute_vindpm =
		of_property_read_bool(np, "ti,bq2589x,use-absolute-vindpm");

	ret = of_property_read_u32(np, "ti,bq2589x,otg_vol", &bq->cfg.otg_vol);
	if (ret)
		return ret;

	ret = of_property_read_u32(np, "ti,bq2589x,otg_current",
				   &bq->cfg.otg_current);
	if (ret)
		return ret;
...
{
	struct bq2589x *bq =
		container_of(work, struct bq2589x, adapter_in_work);
	...
	if (g_battmngr_noti->pd_msg.pd_active) {
		...
		dev_info(bq->dev,
			 "%s:other adapter plugged in,vbus_type is %d\n",
...
{
	struct bq2589x *bq =
		container_of(work, struct bq2589x, adapter_out_work);
	int ret;
	...
	if (ret < 0)
		dev_err(bq->dev,
			"%s:reset vindpm threshold to 4400 failed:%d\n",
			__func__, ret);
	else
		dev_info(bq->dev,
			 "%s:reset vindpm threshold to 4400 successfully\n",
			 __func__);
...
static void bq2589x_ico_workfunc(struct work_struct *work)
{
	...
	if (!ico_issued) {
		...
		if (ret < 0) {
			schedule_delayed_work(&bq->ico_work,
					      HZ); /* retry 1 second later*/
			...
			dev_info(bq->dev,
				 "%s:ICO command issued successfully\n",
				 __func__);
		}
	} else {
		ico_issued = false;
			...
			if (ret == 0) {
				idpm = ((status & BQ2589X_IDPM_LIM_MASK) >>
					BQ2589X_IDPM_LIM_SHIFT) *
				       ...
				       BQ2589X_IDPM_LIM_BASE;
				dev_info(bq->dev,
					 "%s:ICO done, result is:%d mA\n",
					 __func__, idpm);
			}
		}
	}
...
{
	struct bq2589x *bq =
		container_of(work, struct bq2589x, force_work.work);
	u8 val = 0;
...
{
	struct bq2589x *bq =
		container_of(work, struct bq2589x, monitor_work.work);
	u8 status = 0;
	int ret;
	int chg_current;
	int vindpm = 0;
	...
	bq->input_suspend =
		(get_client_vote(bq->input_suspend_votable, USER_VOTER) == 1);
	...
	if (bq->input_suspend) {
		...
		if (get_client_vote(bq->usb_icl_votable, MAIN_CHG_ICL_VOTER) ==
		    100 * 1000)
			ibus_limit = 100;
		else
			dev_err(bq->dev,
				"%s: icl of MAIN_CHG_ICL_VOTER is %d\n",
				__func__,
				get_client_vote(bq->usb_icl_votable,
						MAIN_CHG_ICL_VOTER));
		vote(bq->fcc_votable, BQ_FCC_VOTER, false, 0);
		bq2589x_set_input_volt_limit(bq, 4700);
	} else if (bq->vbus_type == BQ2589X_VBUS_MAXC) {
	...
	dev_info(bq->dev, "%s:pd_active=%d icl=%d\n", __func__,
		 g_battmngr_noti->pd_msg.pd_active, ibus_limit);
	if ((g_battmngr_noti->pd_msg.pd_active == 0) &&
	    (bq->vbus_type == BQ2589X_VBUS_NONE) && (countForcdpdm < 2)) {
	...
	if (bq->batt_psy)
		power_supply_get_property(bq->batt_psy, POWER_SUPPLY_PROP_TEMP,
					  &val);
	if (val.intval < 100)
	...
	dev_info(
		bq->dev,
		"%s:vbus volt:%d,vbat volt:%d,charge current:%d, temp %d, vindpm %d\n",
		__func__, bq->vbus_volt, bq->vbat_volt, chg_current, val.intval,
		vindpm);
...
static bool bq2589x_resolve_dependencies(struct bq2589x *bq)
{
	/*
	 * xm_battmngr consumes this driver's IIO channels, so waiting for
	 * battmngr-owned objects in probe creates a fw_devlink cycle.
	 * Resolve them after the provider has been registered instead.
	 */
	...
	if (!bq->batt_psy || !bq->usb_psy)
		return false;

	return bq_check_vote(bq);
}
...
static void bq2589x_charger_irq_workfunc(struct work_struct *work)
{
	...
	if (!bq2589x_resolve_dependencies(bq)) {
		schedule_delayed_work(&bq->first_irq_work,
				      msecs_to_jiffies(500));
		return;
	}

	/* PD/PPS does not require the conservative BC1.2 detection limit. */
	if (g_battmngr_noti->pd_msg.pd_active)
		vote(bq->fcc_votable, DETECT_FCC_VOTER, false, 0);
	...
	bq->vbus_type =
		(status & BQ2589X_VBUS_STAT_MASK) >> BQ2589X_VBUS_STAT_SHIFT;
	if (((bq->vbus_type == BQ2589X_VBUS_UNKNOWN) ||
	     (bq->vbus_type == BQ2589X_VBUS_NONSTAND))) {
		dev_err(bq->dev,
			"%s:type=%d,status=%d, hz_flag=%d, force_exit_flag=%d, retry_count=%d\n",
			__func__, bq->vbus_type, bq->status, bq->hz_flag,
			bq->force_exit_flag, bq->count);
		if ((!bq->hz_flag) && (!bq->force_exit_flag) &&
		    (bq->count < 3)) {
			dev_info(bq->dev, "%s:float type, rerun bc1.2\n",
				 __func__);
			schedule_delayed_work(&bq->force_work,
	...
	dev_err(bq->dev,
		"%s: part_no=%d, vbus_state=%d, power_good=%d, vbus_type=%d, status=%d\n",
		__func__, bq->part_no, bq->vbus_state, bq->power_state,
		bq->vbus_type, bq->status);

	if ((((bq->part_no == SYV690) &&
	      (!(bq->vbus_state) || !(bq->status & BQ2589X_STATUS_PG))) ||
...
*/
	...
	if (bq->charge_type != POWER_SUPPLY_TYPE_UNKNOWN) {
		if (bq->part_no == SC89890H) {
			...
			bq2589x_use_absolute_vindpm(
				bq, bq->cfg.use_absolute_vindpm);
		}
	}
	...
	if (((last_vbus_type != bq->vbus_type) ||
		...
		g_battmngr_noti->mainchg_msg.msg_type =
			BATTMNGR_MSG_MAINCHG_TYPE;
		battmngr_notifier_call_chain(BATTMNGR_EVENT_MAINCHG,
					     g_battmngr_noti);
...
static irqreturn_t bq2589x_charger_interrupt(int irq, void *data)
{
	...
	dev_err(bq->dev,
		"%s: charger device bq25890 irq detected, revision:%d,bq->vbus_type %d\n",
...
static void bq2589x_first_irq_workfunc(struct work_struct *work)
{
	...
	/*
	 * Keep the conservative FCC vote only while BC1.2 detection is pending.
	 * PD/PPS is already negotiated by TCPM and must not inherit this limit.
	 */
	if (g_battmngr_noti->pd_msg.pd_active) {
		vote(bq->fcc_votable, DETECT_FCC_VOTER, false, 0);
		charger_request_dpdm(bq, false);
...
{
	struct bq2589x *bq =
		container_of(work, struct bq2589x, dump_regs_work.work);
				 ...
				 const struct i2c_device_id *id)
{
	...
	if (!ret && bq->part_no == BQ25890) {
		...
		dev_err(bq->dev,
			"%s: charger device bq25890 detected, revision:%d\n",
			__func__, bq->revision);
	} else if (!ret && bq->part_no == SYV690) {
		bq->status |= BQ2589X_STATUS_EXIST;
		dev_err(bq->dev,
			"%s: charger device SYV690 detected, revision:%d\n",
			__func__, bq->revision);
	} else if (!ret && bq->part_no == SC89890H) {
		bq->status |= BQ2589X_STATUS_EXIST;
		dev_err(bq->dev,
			"%s: charger device SC89890H detected, revision:%d\n",
			__func__, bq->revision);
	} else {
	...
	/*
	 * Publish the IIO provider only after hardware, GPIOs, work items and
	 * IRQ are ready. This breaks the cycle without exposing partial state.
	 */
	ret = bq_init_iio_psy(bq);
	if (ret < 0) {
		dev_err(bq->dev, "Failed to initialize BQ IIO PSY, rc=%d\n",
			ret);
		goto err_free_irq;
	}
